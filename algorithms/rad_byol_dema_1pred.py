...
class RAD_BYOL_DEMA_1PRED(RAD_BYOL):
    def __init__(
        self,
        obs_shape,
        action_shape,
        ...
        self.pcr = utilsmod.PCR_DEMA(self.critic, self.critic_target, encoder_feature_dim, action_shape[0]).to(device)
    ...
    def update_aux(self, obses_q, obses_k, obses_next, actions, L, step):
        ...
        if self.weight != 0:
            ...
            for i in range(self.pred_step):
                ...
                z_c = self.pcr.transition_target(z_c, actions[:, i, :])
                ...
                pred_z_o_pre = self.pcr.predictor_pre(proj_z_o) # single predictor
                pred_z_o_con = self.pcr.predictor_pre(proj_z_o)
