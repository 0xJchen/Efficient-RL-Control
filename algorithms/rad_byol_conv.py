...
class RAD_BYOL_CONV(RAD_BYOL):
    def __init__(
        ...
        self.pcr = utilsmod.PCR_DEC(self.critic, self.critic_target, encoder_feature_dim, action_shape[0]).to(device)
    ...
    def update_aux(self, obses_q, obses_k, obses_next, actions, L, step):
        ...
        if self.weight != 0:
            z_o = self.pcr.encode(obses_q)
            z_c = self.pcr.encode_target(obses_k)  # for consistency

            for i in range(self.pred_step):
                z_p = self.pcr.encode_target(obses_next[:, i, :, :, :])
    ...
    def update(self, replay_buffer, L, step):
        ...
        if step % self.critic_target_update_freq == 0:
            ...
            utils.soft_update_params(
                self.pcr.mlphead, self.pcr.mlphead_target,
                self.encoder_tau
            )
            utils.soft_update_params(
