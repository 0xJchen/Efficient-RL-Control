...
class RAD_BYOL_MV_TEST(RAD_SIMSIAM):
    def update_aux(self, obses_q, obses_k, obses_next, actions, L, step):
        ...
        if self.weight != 0:
            z_o = self.pcr.encoder(obses_q[0])
            ...
            for v in range(self.view):
                z_o = self.pcr.encoder(obses_q[v])
    ...
    def update(self, replay_buffer, L, step):
        ...
        obs_q, obs_k, action, reward, next_obs, not_done = replay_buffer.sample_traj_multi_view(self.augs_funcs, self.pred_step, self.view)
