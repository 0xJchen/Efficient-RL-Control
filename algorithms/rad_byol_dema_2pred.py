...
class RAD_BYOL_DEMA_2PRED(RAD_BYOL):
    def __init__(
        self,
        obs_shape,
        action_shape,
        ...
        self.pcr = utilsmod.PCR_DEMA_TWO_PRED(self.critic, self.critic_target, encoder_feature_dim, action_shape[0]).to(device)
    ...
    def update_aux(self, obses_q, obses_k, obses_next, actions, L, step):
        ...
        if self.weight != 0:
            ...
            for i in range(self.pred_step):
                z_p = self.pcr.encoder_target(obses_next)

                z_o = self.pcr.transition(z_o, actions)
