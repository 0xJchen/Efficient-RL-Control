...
def parse_args():
    ...
    # train
    parser.add_argument('--agent', default='sodadynamic_sac', type=str)
    parser.add_argument('--init_steps', default=1000, type=int)#@wjc changed from 1000 to 100 for testing
    ...
    #@wjc+ht use different batch for sac and our model
    parser.add_argument('--different_batch',default=False, action='store_true') 
    parser.add_argument('--aux_freq',default=1, type=int)
    parser.add_argument('--pred_step',default=1, type=int)
    parser.add_argument('--weight', default=1, type=float)
    parser.add_argument('--view', default=1, type=int)

    # data augs
    parser.add_argument('--data_augs', default='crop', type=str)

    parser.add_argument('--log_interval', default=100, type=int)
    parser.add_argument('--log_dir', default='logs', action='store_true')
...
def evaluate(env, agent, video, num_episodes, L, step, args):
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        filename = args.work_dir + '/' + args.domain_name + '--'+args.task_name + '-' + args.data_augs + '--s' + str(args.seed)+str(args.extra) + '--eval_scores.npy'
...
def main():
    ...
    # print("11111",args.save_model)
    # make directory
    ts = time.gmtime()
    ts = time.strftime("%m-%d", ts)
    env_name = args.domain_name + '-' + args.task_name
    exp_name = args.extra + '-' + env_name + '-' + ts + '-' + args.data_augs + '-im' + str(args.image_size) +'-b'  \
    + str(args.batch_size) + '-s' + str(args.seed)  + '-' + args.encoder_type + '-' + args.agent + '-pred' + str(args.pred_step) \
    + '-w' + str(args.weight) + '-v' + str(args.view) + '-tau' + str(args.encoder_tau)
    ...
    for step in range(args.num_train_steps):
        ...
        if done:
            ...
            if step % args.log_interval == 0:
                ...
                writer.add_scalar(args.work_dir+'/episode_reward', episode_reward, step)
        ...
        episode_reward += reward
        replay_buffer.add(obs, action, reward, next_obs, done_bool, eeo)#follow @ksl
